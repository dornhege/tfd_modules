(define (domain RoomScanning)
  ...
  (:modules
    ...
    (Robot_driveDuration ?this - Robot  ?destination - Pose cost Robot_driveDuration_plannerCall@libtfd_opl_RoomScanning.so)
  )
  (:predicates
  ...
  (:durative-action Robot_drive
    ...
    :duration(= ?duration [Robot_driveDuration ?this ?destination])
    :condition (and
