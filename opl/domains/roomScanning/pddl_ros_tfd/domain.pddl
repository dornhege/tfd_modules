(define (domain RoomScanning)
  ...
  (:modules
    (isReachableFrom ?this - Pose  ?origin - Pose conditionchecker isReachableFrom_plannerCall@libroomScanningModule.so)
    (Robot_driveDuration ?origin ?destination - Pose cost pathCost@libplanner_modules_pr2.so)
  )
  (:predicates
  ...
  (:durative-action Robot_drive
    :parameters(?this - Robot ?origin - Pose ?destination - Pose)
    :duration(= ?duration [Robot_driveDuration ?origin ?destination])
;    :duration(= ?duration 10)
    :condition (and
      (at start (= (Robot_currentPose ?this) ?origin))
      (at start (not (= (Robot_currentPose ?this) ?destination)))
      (at start (not (Robot_busy ?this)))
      (over all ([isReachableFrom ?destination (Robot_currentPose ?this)])))
    :effect (and
