...
; better to assume that one position -> objs stuff and clean somehow
    ...
    (:durative-action grasp
        ...
        (and 
            ...
            ;(forall (?_a - arm) (at end (not (graspable-from ?o ?l ?_a))))
    ...
    (:durative-action detect-objects
        ...
        (and
            ...
            ;(at start (forall (?_a - arm) (= (arm-position ?_a) arm_at_side)))
            (at start (forall (?_a - arm) (arm-drive-pose ?_a)))
            ;(at start (arms-drive-pose))
        )
        :effect 
        (and
