(define (domain tidyup-grasp)
    ...
    ; place ?o in ?a at ?p (while robot is at ?l)
    (:durative-action place
        ; HACK grasp only works with r_arm
	    :parameters (?l - grasp_location ?o - movable_object ?a - r_arm ?p - pose)
        :duration (= ?duration 1.0)
	    :condition (and
            (at start (at-base ?l))
	    ...
	    (and 
            ...
            (at end (not (untucked ?a)))
            (at end (not (post-grasped ?a)))
            ; hmm or reintroduce the pose OF an object as obj-fluent?
            ; have a nicer notion of at-object
            (at end (assign (x ?o) (x ?p)))
            (at end (assign (y ?o) (y ?p)))
            (at end (assign (z ?o) (z ?p)))
            (at end (assign (qx ?o) (qx ?p)))
            (at end (assign (qy ?o) (qy ?p)))
            (at end (assign (qz ?o) (qz ?p)))
            (at end (assign (qw ?o) (qw ?p)))
            (at end (assign (timestamp ?o) (timestamp ?p)))
            (at end (assign (frame-id ?o) (frame-id ?p)))
        )
    )
