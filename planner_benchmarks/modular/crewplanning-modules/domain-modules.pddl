...
(:modules (availableMod ?c - CrewMemeber
    	      		 conditionchecker available@libcrewPlanningModule.so)
	  (effCalls (effectsCalled)
			 effect effectCall@libcrewPlanningModule.so))
...
(:durative-action have_meal
 ...
 :condition (and  (at start ([availableMod ?c]))
