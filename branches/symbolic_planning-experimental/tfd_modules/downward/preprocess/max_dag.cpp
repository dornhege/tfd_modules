...
vector<int> MaxDAG::get_result()
{
    if (debug) {
        for (int i = 0; i < weighted_graph.size(); i++) {
            ...
            for (int j = 0; j < weighted_graph[i].size(); j++)
                cout << " " << weighted_graph[i][j].first << " [weight "
                        << weighted_graph[i][j].second << "]";
            cout << endl;
        }
    }
    ...
    for (int node = 0; node < weighted_graph.size(); node++) {
        ...
        for (int i = 0; i < weighted_edges.size(); i++)
            incoming_weights[weighted_edges[i].first]
                    += weighted_edges[i].second;
    }
    ...
    while (!heap.empty()) {
        ...
        for (int i = 0; i < succs.size(); i++) {
            ...
            if (!done[target]) {
                ...
                heap_positions[target] = heap.insert(make_pair(new_weight,
                        target));
                if (debug)
