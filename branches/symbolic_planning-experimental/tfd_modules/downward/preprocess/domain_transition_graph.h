...
class DomainTransitionGraph
{
    public:
        ...
        typedef vector<
                std::tr1::tuple<const Variable *, double, condition_type> >
                EdgeCondition;
        ...
        virtual void addAxRelTransition(int from, int to,
                const Axiom_relational &ax, int ax_index) = 0;
...
extern void build_DTGs(const vector<Variable *> &varOrder, const vector<
        Operator> &operators, const vector<Axiom_relational> &axioms,
        const vector<Axiom_functional> &axioms_func, vector<
