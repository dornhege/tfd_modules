        ...
        vector<Variable *> variables)
{
        ...
        public:
                    ...
                    const Variable *var, const int &val)
            {
                for (unsigned int j = 0; j < pre_post_start.size(); j++)
                    if (pre_post_start[j].var == var && pre_post_start[j].post
                            == val)
                        return true;
                return false;
            }
    ...
    for (int i = 0; i < prevail_start.size(); i++) {
        cond.insert(std::tr1::make_tuple(prevail_start[i].var,
                prevail_start[i].prev, start_cond));
    }
    ...
    for (int i = 0; i < prevail_overall.size(); i++) {
        if (!self_fulfilled(pre_post_start, prevail_overall[i].var,
            ...
            cond.insert(std::tr1::make_tuple(prevail_overall[i].var,
                    prevail_overall[i].prev, overall_cond));
        }
    }
    ...
    for (int i = 0; i < prevail_end.size(); i++) {
        if (!self_fulfilled(pre_post_start, prevail_end[i].var,
                prevail_end[i].prev)) {
            cond.insert(std::tr1::make_tuple(prevail_end[i].var,
                    prevail_end[i].prev, end_cond));
        }
    }
    //  }
    //  if(type == compressed) {
    ...
    for (int i = 0; i < pre_post_start.size(); i++) {
        if (pre_post_start[i].var->get_level() != level
                && pre_post_start[i].pre != -1) {
            cond.insert(std::tr1::make_tuple(pre_post_start[i].var,
                    pre_post_start[i].pre, start_cond));
            ...
            for (int j = 0; j < pre_post_start[i].effect_conds_start.size(); j++)
                cond.insert(std::tr1::make_tuple(
                        pre_post_start[i].effect_conds_start[j].var,
    ...
    for (int i = 0; i < pre_post_end.size(); i++) {
        if (pre_post_end[i].var->get_level() != level && pre_post_end[i].pre
                != -1) {
            if (!self_fulfilled(pre_post_start, pre_post_end[i].var,
                    pre_post_end[i].pre)) {
                cond.insert(std::tr1::make_tuple(pre_post_end[i].var,
                        pre_post_end[i].pre, end_cond));
            }
        ...
        const Axiom_relational &ax, int ax_index)
{
    ...
    for (int i = 0; i < ax_conds.size(); i++)
        if (true) // [cycles]
            ...
            cond.push_back(std::tr1::make_tuple(ax_conds[i].var,
                    ax_conds[i].cond, ax_cond));
    vertices[from].push_back(trans);
}
...
void DomainTransitionGraphSymb::Transition::dump()
{
    ...
    for (int k = 0; k < condition.size(); k++) {
        cout << " " << std::tr1::get<0>(condition[k])->get_level() << " : "
                << std::tr1::get<1>(condition[k]) << " (" << std::tr1::get<2>(
                condition[k]) << ")" << endl;
    }
}
                    ...
                    if (other_trans.duration.var->get_level()
                        ...
                        if (cond.size() == 0) {
                            ...
                            for (int k = 0; k < cond.size(); k++) {
                                bool cond_k = false;
                                for (int l = 0; l
                                        < other_trans.condition.size(); l++) {
                                    if (std::tr1::get<0>(
                                            other_trans.condition[l])
                                            > std::tr1::get<0>(cond[k])) {
...
void DomainTransitionGraphSymb::dump() const
{
    ...
    for (int i = 0; i < vertices.size(); i++) {
        ...
        for (int j = 0; j < vertices[i].size(); j++) {
            ...
            if (trans.type == start || trans.type == end || trans.type
                    == compressed)
            ...
            for (int k = 0; k < trans.condition.size(); k++)
                cout << "      if "
                        << std::tr1::get<0>(trans.condition[k])->get_level()
                        << " = " << std::tr1::get<1>(trans.condition[k])
                        << " (" << std::tr1::get<2>(trans.condition[k]) << ")"
                        << endl;
        }
    }
}
