    ...
    for (int i = 0; i < count; i++) {
        ...
        if (effect.at(0) != 'm') {
            ...
            if (variables[varNo]->is_functional()) {
                ...
                in >> val >> newVal;
                if (eff_conds)
                    pre_post_start.push_back(PrePost(variables[varNo],
                            ecs_start, ecs_overall, ecs_end, val, newVal));
                else
                    pre_post_start.push_back(PrePost(variables[varNo], val,
                            newVal));
            }
        } else {
            if (eff_conds)
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (effect.at(0) != 'm') {
            ...
            if (variables[varNo]->is_functional()) {
                ...
                in >> assignop >> varNo2;
                if (eff_conds)
                    numerical_effs_end.push_back(NumericalEffect(
                            variables[varNo], ecs_start, ecs_overall, ecs_end,
                            assignop, variables[varNo2]));
                else
                    numerical_effs_end.push_back(NumericalEffect(
                ...
                if (eff_conds)
                    ...
                    pre_post_end.push_back(PrePost(variables[varNo], val,
                            newVal));
            }
        } else {
            if (eff_conds)
...
void Operator::strip_unimportant_effects()
{
    ...
    numerical_effs_start.erase(numerical_effs_start.begin() + new_index,
            numerical_effs_start.end());
    new_index = 0;
    for (int i = 0; i < numerical_effs_end.size(); i++) {
        if (numerical_effs_end[i].var->get_level() != -1)
            numerical_effs_end[new_index++] = numerical_effs_end[i];
    }
    numerical_effs_end.erase(numerical_effs_end.begin() + new_index,
            numerical_effs_end.end());
}
...
void Operator::write_prevails(ostream &outfile, const vector<Prevail> &prevails) const
{
    ...
    for (int i = 0; i < prevails.size(); i++) {
        ...
        if (prevails[i].var->get_level() != -1)
            outfile << prevails[i].var->get_level() << " " << prevails[i].prev
                    << endl;
    }
}
