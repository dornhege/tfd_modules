...
AxiomEvaluator::AxiomEvaluator()
{
    ...
    for (int i = 0; i < g_axiom_layers.size(); i++) {
        ...
        if (g_variable_types[i] == logical) {
            ...
            if (layer < g_first_logic_axiom_layer || g_first_logic_axiom_layer
                    == -1)
    ...
    int max_axiom_layer = max(g_last_logic_axiom_layer,
            g_comparison_axiom_layer);
    ...
    if (g_first_logic_axiom_layer != -1) {
        for (int layer = g_first_logic_axiom_layer; layer
    ...
    for (int var_no = 0; var_no < g_axiom_layers.size(); var_no++) {
        ...
        if (layer > -1 && layer >= g_first_logic_axiom_layer && layer
...
void AxiomEvaluator::evaluate_logic_axioms(TimeStampedState &state)
{
    ...
    if (g_first_logic_axiom_layer != -1) {
        for (int layer_no = g_first_logic_axiom_layer; layer_no
                < g_last_logic_axiom_layer + 1; layer_no++) {
            // Apply Horn rules.
            while (!queue.empty()) {
