        ...
        for (int i = 0; i < prevail.size(); i++) {
            ...
            if (!double_equals(current_val, prevail[i].value)) {
                ...
                if (!child_problem->is_initialized()) {
                    ...
                    child_problem->initialize(base_priority, current_val_int,
                            state);
                }
                int prev_value = static_cast<int> (prevail[i].value);
                LocalProblemNode *cond_node = child_problem->get_node(
                        prev_value);
...
void LocalTransitionDiscrete::on_condition_reached(int /*cond_no*/,
        double cost, const TimeStampedState&)
{
...
void LocalTransitionComp::on_condition_reached(int cond_no, double cost,
        const TimeStampedState& state)
{
    if (!source->opened || source->expanded || conds_satiesfied[cond_no]
            == true) {
        return;
    }
    target_cost += cost;
    ...
    for (int i = 0; i < state.operators.size(); ++i) {
        if (!(state.operators[i].get_name().compare(
            ...
            waiting_time = max(waiting_time, state.operators[i].time_increment
                    - EPS_TIME);
            //                    cout << "wait for " << state.operators[i].get_name() << "....waiting_time: " << get_waiting_time() << endl;
            return true;
        }
    }
    return false;
...
LocalProblemComp::LocalProblemComp(CyclicCGHeuristic* _owner, int the_var_no,
        int the_start_value) :
...
CyclicCGHeuristic::CyclicCGHeuristic(Mode _mode) :
    mode(_mode)
{
    goal_problem = 0;
    goal_node = 0;
