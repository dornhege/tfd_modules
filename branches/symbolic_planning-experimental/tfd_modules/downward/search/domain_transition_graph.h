...
struct LocalAssignment
{
        ...
        LocalAssignment(DomainTransitionGraph *_prev_dtg, int _local_var,
                double _value, condition_type _cond_type) :
            prev_dtg(_prev_dtg), local_var(_local_var), value(_value),
                    cond_type(_cond_type)
        {
            var = -1;
        }
        LocalAssignment(DomainTransitionGraph *_prev_dtg, int _local_var,
                int _var, assignment_op _fop, condition_type _cond_type) :
            prev_dtg(_prev_dtg), local_var(_local_var), var(_var), fop(_fop),
                    cond_type(_cond_type)
        {
            value = -1.0;
        }
...
class ValueTransitionLabel
{
    public:
        ...
        Operator *op;
        ValueTransitionLabel(int the_duration_variable, const vector<
                LocalAssignment> &the_precond, trans_type the_type,
                const vector<LocalAssignment> &the_effect = vector<
                        LocalAssignment> (), Operator* theOp = NULL) :
            duration_variable(the_duration_variable), precond(the_precond),
                    type(the_type), effect(the_effect), op(theOp)
        {
        }
...
class FuncTransitionLabel: public ValueTransitionLabel
{
    public:
        ...
        FuncTransitionLabel(int the_starting_variable,
                vector<LocalAssignment> the_precond,
                vector<LocalAssignment> the_effect, assignment_op the_a_op,
                int the_influencing_variable, int the_duration_variable,
                trans_type type, Operator *the_op) :
            ValueTransitionLabel(the_duration_variable, the_precond, type,
                    the_effect, the_op), starting_variable(
                    the_starting_variable), a_op(the_a_op),
                    influencing_variable(the_influencing_variable)
        {
        }
};
...
class DomainTransitionGraph
{
    public:
        ...
        static void compute_causal_graph_parents_comp(int var,
                map<int, int> &global_to_ccg_parent);
        static void collect_func_transitions(int var,
                map<int, int> &global_to_ccg_parent);
        virtual void dump() const = 0;
    protected:
        int translate_global_to_local(map<int, int> &global_to_ccg_parent,
                int global_var);
};
...
class DomainTransitionGraphSymb: public DomainTransitionGraph
{
    ...
    private:
        ...
        bool add_relevant_functional_vars_to_context(int var_no,
                map<int, int> &global_to_ccg_parent);
        void extend_cyclic_effect(const PrePost& pre_post, vector<
                LocalAssignment>& cyclic_effect,
                map<int, int> &global_to_ccg_parent, const vector<
                        pair<int, int> >& precond_pairs);
...
class DomainTransitionGraphComp: public DomainTransitionGraph
{
    public:
        ...
        virtual void read_data(istream &in);
        void compute_recursively_parents(int var,
                map<int, int> &global_to_ccg_parent);
        void collect_recursively_func_transitions(int var,
                map<int, int> &global_to_ccg_parent);
        virtual void dump() const;
    private:
