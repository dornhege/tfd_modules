        ...
        vector<DomainTransitionGraph *> &transition_graphs)
{
    ...
    for (int i = 0; i < operators.size(); i++) {
            ...
            for (int k = 0; k < pre_posts_to_add.size(); ++k) {
                if (pre_post_end[j].var == pre_posts_to_add[k].first.var) {
                    ...
                    if (intermediate_value
                            < pre_posts_to_add[k].first.var->get_range() - 1) {
                        pre_posts_to_add.push_back(make_pair(Operator::PrePost(
        ...
        for (int j = 0; j < pre_posts_to_add.size(); j++) {
            ...
            if (var_level != -1) {
                ...
                if (pre != -1) {
                    transition_graphs[var_level]->addTransition(pre, post, op,
                            i, pre_posts_to_add[j].second, var_order);
                } else {
                    for (int pre = 0; pre < var->get_range(); pre++)
                        if (pre != post) {
                            transition_graphs[var_level]->addTransition(pre,
                                    post, op, i, pre_posts_to_add[j].second,
                                    var_order);
                        }
                }
            }
        ...
        for (int j = 0; j < numerical_effs_start.size(); j++) {
            ...
            if (var_level != -1) {
                ...
                transition_graphs[var_level]->addTransition(foperator,
                        right_var, op, i, start, var_order);
            }
        }
    ...
    for (int i = 0; i < axioms_rel.size(); i++) {
        ...
        transition_graphs[var_level]->addAxRelTransition(old_val, new_val, ax,
                i);
    }
    ...
    for (int i = 0; i < axioms_func.size(); i++) {
        ...
        DomainTransitionGraphSubterm
                *dtgs =
                        dynamic_cast<DomainTransitionGraphSubterm*> (transition_graphs[var_level]);
        assert(dtgs);
