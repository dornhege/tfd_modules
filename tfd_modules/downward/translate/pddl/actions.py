    ...
    def instantiate(self, var_mapping, init_facts, fluent_facts, init_function_vals,
        ...
        for eff in self.effects:
            eff.instantiate(var_mapping, init_facts, fluent_facts, 
                            init_function_vals, fluent_functions, task, 
                            new_axiom, new_modules, objects_by_type, effects)
        if effects:
              ...
              for (op, pne) in self.duration[dura]:
                if isinstance(pne, conditions.ModuleCall):
                  ...
                  pne.instantiate(var_mapping, init_facts, fluent_facts,
                      init_function_vals, fluent_functions, task, new_axiom, new_modules, result)
                  assert len(result) == 1, "Something when wrong when instantiating cost module"
        ...
        for time,condition in enumerate(self.condition):
            try:
                condition.instantiate(var_mapping, init_facts, fluent_facts, 
                                      init_function_vals, fluent_functions, task,
                                      new_axiom, new_modules, inst_conditions[time])
            except conditions.Impossible:
                return None
        effects = [[],[]]
