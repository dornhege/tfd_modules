    ...
    if not isinstance(rule_head,pddl.NegatedAtom):
      if isinstance(rule_head, pddl.ModuleCall):
        ...
        if my_module:
          ...
          renamings = {}
          for param, arg in zip(my_module.parameters, rule_head.args):
          ...
          for fluent in my_module.effects:
            ...
            module_heads.append(get_fluent_function_predicate(new_fluent))
...
def remove_duration_variable(task):
    def recurse(condition, act, time, duration, pnes):
        if isinstance(condition, pddl.FunctionComparison):
            ...
            return comp       # FIXME: Is this line ever called? - remove?
        else:
...
def remove_arithmetic_expressions(task):
    ...
    for act in task.durative_actions:
        ...
        for (op,exp) in act.duration[0]:
          if isinstance(exp, pddl.ModuleCall):
            dur0.append((op, exp))
          else:
            dur0.append((op, task.function_administrator.get_derived_function(exp)))
...
def condition_to_rule_body(parameters, condition, fluent_preds = None):
  ...
  if not isinstance(condition, pddl.Truth):
    ...
    for part in parts:
      ...
      assert isinstance(part, pddl.Literal) or isinstance(part,pddl.FunctionComparison) or isinstance(part, pddl.ModuleCall), "Condition not normalized"
...
def get_fluent_predicates(task):
  ...
  for action in task.actions:
    for effect in action.effects:
      ...
      elif isinstance(effect.peffect, pddl.ModuleCall):
        pass
      else:
  ...
  for action in task.durative_actions:
    for effect in action.effects:
        for eff in effect:
          ...
          elif isinstance(eff.peffect, pddl.ModuleCall):
            # FIXME: A module effect does nothing (for the view of the planner)
            # Or should we add the affected fluents here? Depends on what this is used for...
            pass
          else:
